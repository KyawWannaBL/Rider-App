...
export default function SupportPage(){
 ...
 const [msg,setMsg]=useState(language === "my" ? "Enterprise အကူအညီ Ticket များကို ဖွင့်နေသည်..." : "Loading Enterprise support tickets...");
 ...
 async function load(){
  ...
  setMsg(tx("Support synchronized with Enterprise CS/Operations queue.","အကူအညီတောင်းဆိုမှုကို Enterprise CS/Operations queue နှင့် ချိတ်ဆက်ပြီးပါပြီ။"));
  setLoading(false);
 }

 async function save(){
  if(!form.message.trim())return setMsg(tx("Support message is required.","အကူအညီတောင်းဆိုသည့် စာသား ဖြည့်ရန်လိုအပ်ပါသည်။"));
  setLoading(true);
