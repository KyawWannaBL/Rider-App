    ...
    <main className="min-h-screen bg-slate-50 p-4">
      <section className="rounded-3xl bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex gap-2">
          <input
            ...
            className="h-12 flex-1 rounded-2xl border px-4 font-bold"
            placeholder="Rider code"
          />
          <button onClick={load} className="rounded-2xl bg-blue-700 px-5 font-black text-white">
            Sync
          </button>
        </div>
      </section>
      ...
      <section className="mt-4 rounded-3xl bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          <Card label="Branch Code" value={branch.branch_code} />
          <Card label="Branch Name" value={branch.branch_name} />
          <Card label="Status" value={branch.status} />
        </div>
      </section>
        ...
        <div className="mt-3 space-y-3">
          {pickups.map((p: any) => (
            <div key={p.id || p.pickup_id} className="rounded-2xl border p-4">
              <p className="font-black text-blue-700">{p.pickup_id || p.pickup_way_id}</p>
              <p className="font-bold">{p.merchant_name}</p>
              <p className="text-sm text-slate-600">{p.pickup_township} · {p.status}</p>
            </div>
          ))}
          {pickups.length === 0 && (
            <p className="rounded-2xl border p-4 text-slate-500">No branch pickups assigned.</p>
          )}
        </div>
      </section>
    </main>
