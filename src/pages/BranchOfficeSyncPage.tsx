...
export default function BranchOfficeSyncPage() {
  ...
  const [message,setMessage]=useState(language === "my" ? "အတည်ပြုထားသော ဌာနခွဲတာဝန်ကို ဖွင့်နေသည်..." : "Loading authenticated branch assignment...");
  const [loading,setLoading]=useState(false);

  async function load(){
    setLoading(true);
    try{
      setMessage(tx("Synchronizing with Enterprise Branch Office...","Enterprise Branch Office နှင့် ချိတ်ဆက်နေသည်..."));
      const result=await loadRiderBranchSnapshot();
      setData(result);
      setMessage(tx("Branch assignment synchronized from Enterprise.","ဌာနခွဲတာဝန်ကို Enterprise မှ ချိတ်ဆက်ပြီးပါပြီ။"));
    }catch(error:any){
  ...
  return <main className="min-h-screen bg-slate-50 p-4 sm:p-6"><div className="mx-auto max-w-6xl space-y-5">
   <section className="rounded-3xl border bg-white p-6 shadow-sm"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.3em] text-blue-600">Britium Rider App</p><h1 className="mt-2 text-3xl font-black">{tx("Branch Office Sync","ဌာနခွဲ ချိတ်ဆက်မှု")}</h1><p className="mt-2 font-bold text-slate-600">{identity.display_name||identity.worker_code||"Rider"} · {identity.worker_code||"-"}</p></div><button onClick={load} disabled={loading} className="inline-flex h-11 items-center gap-2 rounded-2xl bg-blue-700 px-4 text-sm font-black text-white"><RefreshCw className={`h-4 w-4 ${loading?"animate-spin":""}`}/>{tx("Sync","ချိတ်ဆက်ရန်")}</button></div><div className="mt-4 rounded-2xl bg-blue-50 p-3 text-sm font-bold text-blue-900">{message}</div></section>
